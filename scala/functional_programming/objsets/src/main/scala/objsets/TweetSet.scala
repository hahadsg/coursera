...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Nothing = throw new java.util.NoSuchElementException("Empty.mostRetweeted")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    if (p(this.elem)) right.filterAcc(p, left.filterAcc(p, acc.incl(elem)))
  ...
  def mostRetweeted: Tweet = {
    val p: Tweet => Boolean = tweet => tweet.retweets > elem.retweets
    left.filter(p).union(right.filter(p)) match {
      case u: NonEmpty => u.mostRetweeted
      case _: Empty => elem
    }
  }
